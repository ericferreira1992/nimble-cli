...
function printFileSizesAfterBuild(webpackStats, previousSizeMap, buildFolder, maxBundleGzipSize, maxChunkGzipSize) {
    ...
    var assets = (webpackStats.stats || [webpackStats])
        ...
        return stats
            ...
            var size = (0, gzip_size_1.gzipSizeSync)(fileContents);
            ...
            return {
                ...
                size: size,
                sizeLabel: (0, filesize_1.filesize)(size) + (difference ? ' (' + difference + ')' : ''),
            };
        });
    })
        .reduce(function (single, all) { return all.concat(single); }, []);
    assets.sort(function (a, b) { return b.size - a.size; });
    var longestSizeLabelLength = Math.max.apply(null, assets.map(function (a) { return (0, strip_ansi_1.default)(a.sizeLabel).length; }));
    var suggestBundleSplitting = false;
    assets.forEach(function (asset) {
        var sizeLabel = asset.sizeLabel;
        var sizeLength = (0, strip_ansi_1.default)(sizeLabel).length;
...
function getDifferenceLabel(currentSize, previousSize) {
    ...
    var fileSize = !Number.isNaN(difference) ? (0, filesize_1.filesize)(difference) : 0;
...
function measureFileSizesBeforeBuild(buildFolder) {
    return new Promise(function (resolve) {
        (0, recursive_readdir_1.default)(buildFolder, function (err, fileNames) {
            var sizes;
            if (!err && fileNames) {
                sizes = fileNames.filter(canReadAsset).reduce(function (memo, fileName) {
                    ...
                    memo[key] = (0, gzip_size_1.gzipSizeSync)(contents);
                    return memo;
                }, {});
            }
            resolve({
