...
var CLI = exports.CLI = /** @class */ (function () {
    function CLI() {
    }
    Object.defineProperty(CLI, "nimbleVersion", {
        get: function () {
            if (!this._nimbleVersion) {
                ...
                var stable = cp.execSync("".concat(prefix, " version")).toString('utf8').replace(/(\r\n|\n|\r)/gm, '');
                var beta = cp.execSync("".concat(prefix, " dist-tags.beta")).toString('utf8').replace(/(\r\n|\n|\r)/gm, '');
                var alpha = cp.execSync("".concat(prefix, " dist-tags.alpha")).toString('utf8').replace(/(\r\n|\n|\r)/gm, '');
    ...
    CLI.workingInLocalProject = function () {
        ...
        return fs_extra_1.default.existsSync("".concat(projectRootPath, "/nimble.json"));
    };
    CLI.worksPathIsRootProject = function (path) {
        ...
        if (!path.endsWith('/'))
            path += '/';
        if (!fs_extra_1.default.pathExistsSync("".concat(path, "src")) || !fs_extra_1.default.existsSync("".concat(path, "package.json"))) {
            return false;
        }
        if (!fs_extra_1.default.existsSync("".concat(path, "nimble.json"))) {
            return false;
        }
        var packageFile = null;
        try {
            packageFile = require("".concat(path, "package.json"));
        }
        catch (_a) { }
